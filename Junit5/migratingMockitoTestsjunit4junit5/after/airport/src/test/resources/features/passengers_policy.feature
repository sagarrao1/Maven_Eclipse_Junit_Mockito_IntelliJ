Feature: Passengers Policy
  ...
  Scenario Outline: Flight with usual passengers
    Given there is a flight having number "<flightNumber>" and <places> places with passengers defined into "<file>"
    When we have usual passengers
    Then you can remove them from the flight
    And add them to another flight

    Examples:
      |flightNumber  | places | file                       |
